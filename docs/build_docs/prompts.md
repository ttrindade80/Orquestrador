...
```
Leia docs/build_docs/instruction.md antes de qualquer coisa.

Adicione ao docs/build_docs/to_do.md um novo item com o formato já
...
completamente fechada no módulo proprietário do domínio ou no contrato
aplicável, e não sobrar nenhuma pergunta em aberto. Caso contrário,
classifique como `bloqueado_decisao` e descreva exatamente o que falta
decidir.
...
```
Leia docs/build_docs/instruction.md e
docs/build_docs/to_do.md antes de qualquer coisa.
...
Depois que eu escolher, releia a origem indicada no item (ADR, contrato ou
módulo proprietário declarado no campo "Origem") pra confirmar que a
decisão está mesmo fechada e sem ambiguidade. Se encontrar algo que precisa
de confirmação minha antes de prosseguir, pergunte — não assuma.
...
- módulo(s) proprietário(s) relevante(s) (localizar via docs/nomenclatura/00_INDICE.md); docs/NOMENCLATURA.md como fachada de navegação
...
1. Nenhuma contradição entre o que o módulo proprietário ou contrato
   diz e o que o ADR/JSON efetivamente registram.
...
4. Nomenclatura de arquivo e campo segue a política de
   docs/nomenclatura/02_ARTEFATOS_CONFIGURACAO_E_RUNTIME.md (sem abreviação que misture termos distintos).
